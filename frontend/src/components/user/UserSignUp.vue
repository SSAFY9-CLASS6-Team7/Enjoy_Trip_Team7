    ...
    <div class="main-container">
      ...
      <div class="id-input-container">
        <input
          type="text"
          class="id-input"
          :class="{ success: idFlag === true, fail: idFlag === false }"
          v-model="id"
          @change="idEmpty"
        />
      ...
      <div
        class="id-check-result"
        :class="{ 'result-success': idFlag === true, 'result-fail': idFlag === false }"
      >
        {{ idCheckResult }}
      </div>
      ...
      <div
        class="password-check-result"
        :class="{ 'result-success': passwordFlag === true, 'result-fail': passwordFlag === false }"
      >
        {{ passwordCheckResult }}
      </div>
      ...
      <div class="gender-input-container">
        <div class="male-container">
          <input
            ...
            hidden
          />
          <label class="gender-radio" for="male" :class="{ 'male-selected': gender == 'M' }"
            >남 자</label
          >
        </div>
        <div class="female-container">
          <input
            ...
            hidden
          />
          <label class="gender-radio" for="female" :class="{ 'female-selected': gender == 'F' }"
            >여 자</label
          >
        </div>
      </div>
  ...
  methods: {
    idCheck() {
      ...
      if (this.id.length > 15 || this.id.length < 1) {
        ...
        if (!alphanumericRegex.test(this.id)) {
          ...
          // 중복 체크 로직
          axios.get(process.env.VUE_APP_MY_BASE_URL + '/user/' + this.id).then((response) => {
    ...
    signUp() {
      if (
        ...
        axios.post(process.env.VUE_APP_MY_BASE_URL + '/user', user);
        this.modal = true;
      } else {
